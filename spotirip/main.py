...
from streamrip.media import Track, PendingTrack, Album, PendingAlbum
...
async def download_track(c: DeezerClient, ptr: PendingTrack,
                         limiter: aiolimiter.AsyncLimiter) -> None:
    clogger = logger.getChild("download_track")
    clogger.debug(f"entering")

    async with limiter:
        tr = await ptr.resolve()
        logger.debug(f"Resolved track!")
        if tr is None:
            return None
...
async def download_album(c: DeezerClient, db: Database, config: Config,
                         album_id: str, album_filter: AlbumFilter,
                         limiter: aiolimiter.AsyncLimiter) -> None:
    ...
    logger.debug(f"resolved album")
    ptrs = ral.tracks
    if not album_filter.whole:
        ptrs = [t for t in ral.tracks if t.id in album_filter.tracks]

    await asyncio.gather(*[download_track(c, ptr, limiter) for ptr in ptrs])
        
def read_ids_from_file(file_path: Path) -> List[str]:
    with open(file_path) as fp:
        return [s.removesuffix('\n') for s in fp.readlines()]

async def main(config_path: Path, debug: bool, playlists: Path,
               albums: Path):
    ...
    logger.info(f"Logged into Deezer!")
    ...
    try:
        ...
        logger.warn(f"Failed to find track on Deezer: {e}")

    for track_id, album_id in pl_tr_infos:
        if not albums.get(album_id):
            albums[album_id] = AlbumFilter(False, set([track_id]))
        else:
    ...
    await asyncio.gather(*[download_album(c, db, config, album, album_filter, limiter) for album, album_filter in albums.items()])
